...
!     real(adqt), device, allocatable  :: STime(:,:,:)     ! time dependent source new layout:(ig,c0+c,zone)
     ...
     ! during sweep, there is some work that is just geometry work.
     ! split sweep into two parts.  A_fp goes in, and omega_A_fp, etc.p
     ! then get fed into second half of sweep kernel
